...
(() => {
  ...
  function makeCardsInteractive(root = document) {
    const selector = ".info-card, .gallery-card, .site-card, .topic-card, .stat-card, .product-card, .cart-item, .summary-card, .process-steps article, .checkout-card";
    root.querySelectorAll?.(selector).forEach((card) => {
      if (card.dataset.peInteractive === "true") return;
      card.dataset.peInteractive = "true";
      if (!card.hasAttribute("tabindex")) card.setAttribute("tabindex", "0");
      card.addEventListener("click", (event) => {
        if (event.target.closest("a, button, input, select, textarea, summary")) return;
        card.classList.toggle("pe-card-active");
      });
